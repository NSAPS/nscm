...
var job_id = 'op_02020_Long_Term_Planning_list_roh';
...
function setHeader(GridObj) {        
	...
	GridObj.AddHeader("ITEM_ID"				,"품목번호"       	,"t_text" 		,20		,70  ,false); //0   
 	...
 	GridObj.AddHeader("TOT_LEAD_TIME"		,"리드\n타임"     	,"t_number" 	,100.3	,0   ,false); //0
 	GridObj.AddHeader("BASE_UOM"			,"기본\n단위"     	,"t_text" 		,100	,33  ,false); //0
 	GridObj.AddHeader("BASE_STOCK"			,"현재고"      		,"t_number" 	,100.3	,55  ,false); //0   
 	GridObj.AddHeader("PROG_STOCK"			,"진행중 재고"   		,"t_number" 	,100.3	,70  ,false); //0   
 	GridObj.AddHeader("TOT_STOCK"			,"총재고"      		,"t_number" 	,100.3	,75  ,false); //0
 	GridObj.AddHeader("SEL_DMD"				,"선택\n구분"     	,"t_combo"		,100	,65   ,true); //0	
 	GridObj.AddHeader("STD_STOCK"			,"기준 재고"     		,"t_number" 	,100.3	,70   ,true); //0   
 	GridObj.AddHeader("SAFETY_STOCK"		,"안전 재고"     		,"t_number" 	,100.3	,60  ,false); //0   
 	...
 	GridObj.AddHeader("MIN_LOT_SIZE"		,"최소발주량"   		,"t_number" 	,100.3	,80   ,true); //0   
 	GridObj.AddHeader("PR_DATE_NO"			,"발주\n시기" 		,"t_number" 	,100.3	,0    ,true); //0   
 	GridObj.AddHeader("PR_QTY"				,"PR\n수량"      	,"t_number" 	,100.3	,0    ,true); //0   
 	GridObj.AddHeader("ENTR_DATE"			,"입고일"      		,"t_text" 		,100	,75   ,true); //0   
 	GridObj.AddHeader("EDIT_FLAG"			,"EDIT_FLAG"    	,"t_text" 		,100	,0    ,true); //0   
 	GridObj.AddHeader("PR_NO"				,"PR 번호"     		,"t_text" 		,100	,0    ,true); //0   
 	GridObj.AddHeader("IF_MSGS"				,"IF 메세지"     		,"t_text" 		,100	,0    ,true); //0   
	...
	GridObj.SetColCellBgColor('MIN_LOT_SIZE',	color_edit_col);//발주단위
...
function setHeader2(GridObj2) {        
  	...
  	GridObj2.AddHeader("CREDIT"		,"선적일" 		,"t_text" 	,100		,67  ,false); //0   
	...
	if(mode == "search2") {
		if(GridObj2.GetStatus() == "true") {
			GridObj2.SetGroupMerge(	'ITEM_ID,ITEM_NAME,PR_NO,PR_REL,PR_QTY,PO_NO,PO_SEQ,PO_DATE,PO_QTY');
		  	GridObj2.AddSummaryBar('SUMMARY', '합계', 'summaryall', 'sum', 'PR_QTY,PO_QTY,SHIP_QTY');
...
/*┌──────────────────────────────────┐
  ...
  └──────────────────────────────────┘*/
   function doQuery() 
   {
       ...
       var domain1		= document.all.domain1.value;
       
       //넘겨줄 값들을만든다.( 파라미터 정의 부분 )
       GridObj.SetParam("mode",			 "search");
       GridObj.SetParam("item_type",	item_type);
       GridObj.SetParam("domain1",		   domain1);
...
function doChange2(){
	...
	commonUtil.getSelQeury("item_type",item_type,"sel_dmd_list_2", { 
	callback:function(arrList){
		var selected_row = 0;
		
		in_div = "<select name=\"domain1\"  >";
		for(var i=0 ; i < arrList.length ; i++){

			in_div +=	"<option value="+arrList[i][0];

			if(document.frm.domain1.value == arrList[i][0]) {
				in_div += " selected";
				selected_row = i;
			}
		...
		divDomain1Combo.innerHTML = in_div;
		// 품목cnt정보를 셋팅한다.
