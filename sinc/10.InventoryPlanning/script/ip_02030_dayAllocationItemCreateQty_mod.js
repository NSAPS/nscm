////////////////////////////////////////////////////////////
// 프로그램ID      : ip_02030_dayAllocationItemCreateQty_mod.js
// 프로그램명      : 일간공급할당관리
// 개발자          : 남웅용
// 개발일자        : 2008-05-01
//
...
// ---------  ----------  --------  ------------------------------------
// 1.0        2008-05-01  남웅용    	create
// 2.0 		  2009-10-09  남웅용		긴급공급할당시 영업에서 거래처 할당(SDORT201)을 진행했으면 정정금지!
// 										
////////////////////////////////////////////////////////////
// 제품 검색 POPUP
...
function openRequestAllocReasonListPopup(col_no, dw3_row, day_cnt) { 	
	...
	var in_paramKey = "sel_work_date!%!sel_sales_id!%!sel_dc_id!%!sel_item_id";
	var in_paramCode = this_work_date+"!%!"+this_dw3_sales_id+"!%!"+this_dw3_dc_id+"!%!"+in_alloc_item;
...
GoDelete = function(service,day_cnt) {
	...
	var in_alloc_item = document.frm.in_alloc_item.value;
	var old_alloc_item = document.frm.old_alloc_item.value
	var insel_allocReason = document.frm.insel_allocReason.value
	var this_cnfm_date = "";
	...
	// 현재 조회하고 있는 품목을 저장해 놓는다! 작업중에 품목이 바뀌는 것 방지!!!
	if(Number(in_alloc_item) != Number(old_alloc_item)){
		alert("선택품목과 조회된 내용이 다릅니다! 다시 조회한 후 작업하십시요!");	
		return;
	}
	...
	for(var i = 0 ; i < day_cnt; i++){
		...
		// 단, 긴급공급할당 삭제인 경우는 가능하도록 한다. 2008.11.22 SCM팀 요청
		if((document.frm.chk_alloc_request[i].value == "Y" && insel_allocReason != "11")
			&& document.frm.sel_alloc_flag[i].value == "Y"){
...
// 판매계획 없어도 배정가능하도록 조회 -> checkbox값을 hidden data에 반영
function doCheckPlanYN(obj){

	if(obj.checked){
			document.frm.chk_plan_yn.value = "Y";
	}
	else{
			document.frm.chk_plan_yn.value = "N";
	}
}
...
function	createAllocQty(day_cnt){
	...
	var old_alloc_item = document.frm.old_alloc_item.value
	var insel_allocReason = document.frm.insel_allocReason.value
	var in_alloc_reason_comment = document.frm.in_alloc_reason_comment.value
	var this_cnfm_date = "";
	var run_cnt = 0;
	...
	if( insel_allocReason == "" || insel_allocReason == null 
			|| in_alloc_reason_comment == "" || in_alloc_reason_comment == null) {
		alert("공급할당 이력을 관리하기 위해 '공급할당사유'와 COMMENT를 입력하시기 바랍니다.");
		return;
	} 
	...
	for(var i = 0 ; i < day_cnt; i++){
		...
		// 단, 긴급공급할당인 경우는 가능하도록 한다. 2008.11.22 SCM팀 요청
		if((document.frm.chk_alloc_request[i].value == "Y" && insel_allocReason != "10") 
			&& document.frm.sel_alloc_flag[i].value == "Y"){
	...
	/* 긴급공급할당의 경우 익일에 대해서는 NFOS에서 거래처 할당이 진행중인지 확인한다. */
	if(insel_allocReason == "10"){
		commonUtil.getCodeList("in_item_id",in_alloc_item,"ip_02030_Alloc_Available_Check", { 
		callback:function(arrList){
			if(arrList[0]==1) {
				alert("NFOS에서 공급할당작업이 진행되고 있습니다. 정정할 수 없습니다!");
				return;
			}
...
//   -> 저장하기까지는 실제할당량은 알수없다.
function	doChange_dw1(obj,objnum,col_no,day_cnt){
	...
	if(this_alloc_box > this_base_stock+this_receipt_box){
		...
		if(confirm("할당량이 (기초재고+입고예정)= "+this_expt_box+"을 "+this_term_box+"만큼 초과했습니다! \n" +
					   "그래도 할당하시겠습니까?") != 1 ) {
	...
	var rowNo_DW2 = main_tbody.rows.length;
	var dw2_sales_plan = 0;
	var dw2_new_dc_alloc_box = 0;

	for(var i = 0 ; i < rowNo_DW2 ; i++){

		// 영업지점의 판매계획+판촉계획을 계산한다.
		dw2_sales_plan = Number(document.frm.sel_dc_sales_plan[i*day_cnt+col_no].value) + Number(document.frm.sel_dc_promo_plan[i*day_cnt+col_no].value);
		
		// 판매계획이 0이 아니면...
		if(dw2_sales_plan > 0) {
			
			// 전체 할당율을 영업지점 공급할당율로 적용한다.
			document.frm.sel_dc_alloc_rate[i*day_cnt+col_no].value 	= numberFormat(this_alloc_rate);
			document.frm.old_dc_alloc_rate[i*day_cnt+col_no].value 	= this_alloc_rate;
			
			// 영업지점 공급할당량을 변경한다.
			dw2_new_dc_alloc_box = Number(Math.round(this_alloc_rate*dw2_sales_plan/100));
			document.frm.sel_dc_alloc_box[i*day_cnt+col_no].value 	= numberFormat(dw2_new_dc_alloc_box);
			document.frm.old_dc_alloc_box[i*day_cnt+col_no].value 	= dw2_new_dc_alloc_box;
		}
		/* 정정 flag를 'Y'로 한다 */
		document.frm.dw2_modi_flag[i*day_cnt+col_no].value 	= 'Y';
	}
	
//	document.frm.chk_alloc[col_no].checked = true;
//	document.frm.chk_alloc_request[col_no].value	= "Y";
	alert(this_alloc_rate+"%"+"로 할당완료 되었습니다! 저장하면 반영됩니다.");
...
function	doChange_dw2(obj, objnum, col_no, dw2_row, day_cnt){
	...
	if(this_dc_alloc_box > old_dc_alloc_box){ //정정수량이 이전수량보다 큰경우!
		...
		if(this_dc_base_stock - this_dc_alloc_box < 0){ // DC의 기초재고로 공급할당 추가량을 cover하지 못할때
			...
			if(this_tot_reqt_box > this_remn_tran_box ){
				...
				if(confirm("수송가능량을 초과했습니다. 정정할 수 없습니다! \n" +
							   "그래도 할당하시겠습니까?") != 1 ) {
	...
	/* 정정 flag를 'Y'로 한다 */
	document.frm.dw2_modi_flag[dw2_row*day_cnt+col_no].value 	= 'Y';
...
function	doChange_dw3(obj, objnum, col_no, dw3_row, day_cnt){
	...
	if(this_sl_alloc_rate > old_sl_alloc_box){ //정정수량이 이전수량보다 큰경우!
		...
		if(tot_sl_alloc_box > this_dc_expt_stock){
			...
			if(confirm("해당 배송지점의 (기초재고+보충필요량)= "+this_dc_expt_stock+"을 "+sl_alloc_term_box+"만큼 초과했습니다. \n" +
					   "그래도 할당하시겠습니까?") != 1 ) {
...
function	doChange_dw3_add(obj, objnum, col_no, dw3_row, day_cnt){
	...
	if(this_add_alloc_box > old_add_alloc_box){ //정정수량이 이전수량보다 큰경우!
		...
		if(tot_sl_alloc_box > this_dc_expt_stock){
			...
			if(confirm("해당 배송지점의 (기초재고+입고예정)= "+this_dc_expt_stock+"을 "+sl_alloc_term_box+"만큼 초과했습니다! \n" +
					   "그래도 할당하시겠습니까?") != 1 ) {

//			alert("해당 배송지점의 (기초재고+입고예정)= "+this_dc_expt_stock+"을 "+sl_alloc_term_box+"만큼 초과했습니다!");
				// 할당율, 할당량 변경 불가
...
// DW1 엔터키 입력
function moveNextBox_DW1( obj,objnum,col_no,day_cnt ) {
	// TAB(9) or ENTER(13)
	if( event.keyCode == "13" ) {
		doChange_dw1(obj,objnum,col_no,day_cnt);
	}
}
