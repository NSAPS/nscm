...
//ERP Å×ÀÌºí ³Ñ±æ °ªµé°ú ¸ÅÄª ÇÊ¿ä
	...
	GridObj.AddHeader("DC_ID"				,"DC_ID"    			,"t_text" 		,100    ,0  ,false);
 	GridObj.AddHeader("DC_NAME"				,"DC¸í"   				,"t_text" 		,100	,130 ,false); //0   
	GridObj.AddHeader("CALOG_BOX_STOCK"		,"Ä¶·Î±×\nBOXÀç°í"   		,"t_number" 	,100.3	,60  ,false); //0
	GridObj.AddHeader("CALOG_PLT_STOCK"		,"Ä¶·Î±×\nPLTÀç°í"   		,"t_number" 	,100.3	,60  ,false); //0
	...
	GridObj.AddHeader("SEQ"					,"Â÷¼ö"   				,"t_number" 	,100.3	,60  ,true); //0
	GridObj.AddHeader("IF_FLAG"				,"»óÅÂ"   				,"t_text" 		,100	,100 ,false); //0
	GridObj.AddHeader("RE_ORDER_FLAG"		,"Áßº¹¿©ºÎ"   			,"t_text" 		,100	,0   ,false); //0
    ...
    function GridEndQuery() 
    {
        ...
        if(endMode == "search") //Á¶È¸°¡ ¿Ï·áµÈ °æ¿ì
        {
            ...
            {  
				...
				GridObj.AddSummaryBar('SUMMARY1', '¼Ò°è', 'DC_ID', 'sum', 'DC_ALLOC,DC_ALLOC_PLT');
				GridObj.AddSummaryBar('SUMMARY2', 'ÇÕ°è', 'summaryall', 'sum', 'DC_ALLOC,DC_ALLOC_PLT');
...
function GoDelete(){
	...
	var rowCount = GridObj.GetRowCount();
		...
		if(confirm("¼±ÅÃ ÇÏ½Å Ç°¸ñÀ» »èÁ¦ÇÏ½Ã°Ú½À´Ï±î?") == 1 ) {
	...
	for(var i=0;i<GridObj.GetRowCount();i++){
		...
		if( GridObj.GetCellValue("SELECTED",i)=="1") {
			if( GridObj.GetCellValue("IF_FLAG", i)=="Àü¼ÛÁß" || GridObj.GetCellValue("IF_FLAG", i)=="Àü¼Û¿Ï·á" ){
			alert("ERP Àü¼Û¿Ï·á ¶Ç´Â Àü¼ÛÁßÀÎ ¿À´õ´Â »èÁ¦ÇÒ ¼ö ¾ø½À´Ï´Ù.");
			return;
			}
		}
...
//	
//	
//		if(confirm("¼±ÅÃ ÇÏ½Å »óÇ°ÀÇ ERP Àü¼ÛÀ» È®Á¤ÇÏ½Ã°Ú½À´Ï±î?") == 1 ) {
//			
//		}
//		else{
//			
