//############################################################
//## 프로그램ID : sc_11020_dailyProductionPlanAnalysisNew_popup
...
//## ---------  ----------  --------  ------------------------------------
//## 1.0        2008-11-27  정재교          sc_11020_dailyProductionPlanAnalysisNew_popup.js 개발
//##
//##
//############################################################
...
GoSave = function() {
	...
	var value = document.frm.plant_id.value + "!%!" + document.frm.proc_id.value + "!%!" 
				+ document.frm.input_start_dttm_time.value + "!%!" + document.frm.input_start_dttm_date.value;
	commonUtil.getCodeInfo(param, value , "work_diary_check", { 
		callback:function(arrList){
			if( arrList.length < 1){	
				...
				var param = "plant_id!%!version!%!seq!%!item_id!%!input_proc_id!%!proc_id!%!input_start_dttm_time!%!input_start_dttm_date!%!cat_id";
				var value = document.frm.plant_id.value + "!%!" + document.frm.version.value + "!%!"
							+ document.frm.seq.value + "!%!" + document.frm.item_id.value + "!%!"
							+ document.frm.input_proc_id.value + "!%!" + document.frm.proc_id.value + "!%!"
							+ document.frm.input_start_dttm_time.value + "!%!" + document.frm.input_start_dttm_date.value + "!%!"
							+ document.frm.cat_id.value;
				
				// 이동할 shift에 수량이 있는지 체크. 있으면 메시지 표시
				commonUtil.getCodeInfo(param, value , "semi_move_target_check", { 
					callback:function(arrList){
						if( arrList.length > 0){	
							if(!confirm("이동할 SHIFT에 수량이 있습니다. 이동 하시겠습니까?")){
								return;
							}
							else{
								// service_id 저장 -- 수량 있음( 수량 통합 )
								var service = "sc_11020_dailySemifinishedProductPlanAnalysisNew_popup_save02_comp";
							...
							// service_id 저장 -- 수량 없음( 이동 처리 )
							var service = "sc_12020_dailySemifinishedProductPlanAnalysisNew_popup_save01_comp";
							frameSetService.location = "service.do?_moon_service=set_service_id&amp;user_service=" + service;				
							document.frm._moon_service.value = service;
							document.frm.action = "service.do";
							document.frm.target = "_self";
