...
//	oCell10.align = "center"; 	oCell10.width = document.frm.e_time.value;  // 종료일시 time
	...
	oCell2.innerHTML = "<select " 
        			    ...
        			    + "<input type=\"hidden\" name=\"input_version\" value=\"" + document.frm.semi_version.value + "\">";						
...
//		arrData[8]	= document.frm.input_end_time.value; //종료일시 TIME
	...
	if(left_tbody.rows[tableLen-1] && (tableLen-1) > 0)
		arrData[10]  = left_tbody.rows[tableLen-1].style.backgroundColor; // 체크 color
	else
		arrData[10]  = left_tbody.style.backgroundColor; // 체크 color
...
//		document.frm.input_end_time.value = arrData[8]; // 종료일시 time
	...
	if(left_tbody.rows[tableLen-1])
		left_tbody.rows[tableLen-1].style.backgroundColor = arrData[10]; // 체크 color
	else
		left_tbody.style.backgroundColor = arrData[10]; // 체크 color
	if(main_tbody.rows[tableLen-1])
		main_tbody.rows[tableLen-1].style.backgroundColor = arrData[10]; // 체크 color
	else
		main_tbody.style.backgroundColor = arrData[10]; // 체크 color
			...
			if( arrList.length < 1){	
				...
				if(tabLength == 1){
					value += document.frm.input_plant_id.value + "!%!" + document.frm.input_version.value + "!%!"
							...
							+ document.frm.input_shift_type.value + "!%!" + "" + "!%!" 
							+ "" + "!%!" + document.frm.input_cat_id.value;	
				}
				else{
					value += document.frm.input_plant_id[tabLength-1].value + "!%!" + document.frm.input_version[tabLength-1].value + "!%!"
							...
							+ document.frm.input_shift_type[tabLength-1].value + "!%!" + "" + "!%!" 
							+ "" + "!%!" + document.frm.input_cat_id.value;	
				}
...
function doChangeItemList(obj){
	...
	commonUtil.getCodeInfo( param, value , "semi_input_plant_proc_of_item_list", { 
		callback:function(arrList){
			if( arrList.length > 0){
