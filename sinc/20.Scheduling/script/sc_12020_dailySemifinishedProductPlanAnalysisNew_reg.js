...
GoSave = function(service) {
	...
	if( document.frm.flag.value == "M" ){
		...
		document.frm.submit();				
// 조회할때는 아직 데이터가 없어서 조회안됨.
//		opener.GoSearch("sc_12020_dailySemifinishedProductPlanAnalysisNew_list");
	}
	else{
		var tabLength = left_tbody.rows.length;
		var service = "sc_12020_dailySemifinishedProductPlanAnalysisNew_reg_save02_comp";
		...
		commonUtil.getCodeInfo(param, value , "work_diary_check", { 
			callback:function(arrList){
				if( arrList.length < 1){	
					...
					var param = "cat_id!%!plant_id!%!version!%!seq!%!item_id!%!input_proc_id!%!proc_id!%!input_start_dttm_time!%!input_start_dttm_date";
					var value = document.frm.cat_id.value + "!%!" + document.frm.plant_id.value + "!%!" + document.frm.version.value + "!%!"
