...
function checkDup() {
	...
	var cd_grp = document.frm.cd_grp.value;
	
	if( cd_grp == "" || cd_grp == null ) {
		alert("코드 그룹을 먼저 입력하세요.");
		document.frm.cd_grp.select();
		return;
	}
	else {
		commonUtil.checkKeyValue(cd_grp, "cd_grp_dup_check_in_code_mst", checkDupDo);
	}
...
function checkDupDo(checkResult) {
	...
	if( Number(checkResult) > 0 ) {
		alert("이미 등록되어 있는 코드 그룹입니다.");
		document.frm.cd_grp.select();
		return;
	}
	else {
		alert("등록 가능한 코드 그룹입니다.");
		// checkDupFlag 에 cd_grp 값 넣음
		// checkDupFlag == cd_grp 로 중복체크 여부 확인
		document.frm.checkDupFlag.value = document.frm.cd_grp.value;
	}
...
function GoSave( service ) {
	...
	// 코드 그룹 입력 체크
	if( document.frm.cd_grp.value == null || document.frm.cd_grp.value == "" ) {
		alert("코드 그룹을 입력하세요.");
		document.frm.cd_grp.select();
		return;
	}
	...
	// 코드 그룹 중복 체크 여부 확인
	if( document.frm.checkDupFlag.value != document.frm.cd_grp.value ) {
		alert("코드 그룹 중복 체크를 하세요.");
		return;
	}
