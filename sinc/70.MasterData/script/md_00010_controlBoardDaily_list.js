////////////////////////////////////////////////////////////
// 프로그램ID      : md_00010_controlBoardDaily_list.js
// 프로그램명      : 일간계획(Contol Board)
// 개발자          : zionex
// 개발일자        : 2008-05-01
//
...
// 2.0 		  2009-09-10  남웅용		생산계획 ERP전송시 ERP전송완료 후 생산오더 또는 계획오더까지
//									완전히 생성된 후 처리될 수 있도록 수정. 
//									--> 완제품인 경우 계획에 동일공장(반제품포함)의 계획이 I/F되고 있으면 안된다. 
// 									    단, 반제품계획의 경우는 공장상관없이 I/F되고 있는 것이 없어야 한다.
//									function execIFErp(), md_00010_ERP_IF_available_check사용
// 3.0 		  2010-03-11  남웅용	공장할당을 제외한 일괄실행기능 기존기능 사용가능하도록 함.
//									단 I/F popup과 동시에 셋팅을 위해 I/F pop-up후 실행버튼을 눌러야 함.
////////////////////////////////////////////////////////////
...
function viewBasic( objBtn ) {
	...
	cat_id = "DP_" + cat_id + "_MD"; // DP : DAILY PLAN, MD : MASTER DATA
...
function execIF( objBtn ) {
	...
	if(cat_id == "FA"){
		service_url += "&faSecondFlag=" + "RATE";  // 공장할당 I/F시 default로 비율이 선택되도록 한다.
	}
...
function addPlan( objBtn ) {
	...
	// 선택
	oCell0.innerHTML = "<input type=\"radio\" name=\"selPlan\" onClick=\"checkPlan(this); \" style=\"border:0px; \"> "
...
function execIFErp( objBtn ) {
	...
	document.frm.btnToErp[rowIdx].disabled = true; // 재전송 방지를 위해 버튼 disable!
	...
	// 버젼이 없는 경우는 계획이 없는 경우이다! 이때는 ERP전송은 의미가 없고 상태값만 이상해진다!
	if(version == null || version == "") {
		alert("전송할 버젼이 없습니다! 다시 확인하십시요.");
		return;		
	}

	objBtn.className = "button1_1";
	
	if( cat_id == "PS" || cat_id == "SS" || cat_id == "ES" ) {

		// ver 2.0 변경사항 생산계획의 ERP전송후 생산오더나 계획오더가 모두 생성된 후 실행될 수 있도록 한다.
		//	--> 완제품인 경우 계획에 동일공장(반제품포함)의 계획이 I/F되고 있으면 안된다. 
		//	    단, 반제품계획의 경우는 공장상관없이 I/F되고 있는 것이 없어야 한다.
		var in_sel_name = "cat_id"+"!%!"+"run_date"+"!%!"+"run_seq"+"!%!"+"plan_step"+"!%!"+"plant_id";
		var in_sel_value = cat_id +"!%!"+run_date+"!%!"+run_seq+"!%!"+plan_step+"!%!"+plant_id;
			
		commonUtil.getCodeInfo(in_sel_name,in_sel_value,"md_00010_ERP_IF_available_check", { 			
			callback:function(arrList){
				var erp_if_available = "N";
				if( arrList.length == 1 ) {
					erp_if_available = arrList[0][1];
				}
...
function viewIFErp( objBtn ) {
	...
	if(cat_id == 'PS' || cat_id == 'SS') {
		var service_url = "service.do?_moon_service=md_00010_controlBoardDaily_viewIfErp2_pop&_moon_perpage=200&_moon_pagenumber=1"; 
		service_url += "&cat_id=" + cat_id + "&sub_cat=" + sub_cat + "&rowIdx=" + rowIdx; 
		service_url += "&period_type=" + period_type + "&run_date=" + run_date + "&run_seq=" + run_seq + "&plan_step=" + plan_step;
		service_url += "&plant_id=" + plant_id;
		
		var pop_win_style = "titlebar=no, menubar=no, toolbar=no, status=yes, scrollbars=no, resizable=yes, "
							+ "width=840, height=450, top=0, left=0";
