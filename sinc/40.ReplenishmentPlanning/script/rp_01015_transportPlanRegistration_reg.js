...
function addRow( objBtn ) {
	...
	oCell3.align = "left";   	oCell3.width = "280px"; // 제품명
...
function memLastRow() {
	...
	if( document.frm.routeClass[tableLen-2] ) {
		arrData[0] 	= document.frm.trans_date[tableLen-2].value;   // 수송일자
		arrData[1] 	= document.frm.item_id[tableLen-2].value;      // 제품코드
		arrData[2] 	= document.frm.item_name[tableLen-2].value;    // 제품명
		arrData[3] 	= document.frm.base_stk_plt[tableLen-2].value; // 기본재고상차 PLT
		arrData[4] 	= document.frm.base_stk_box[tableLen-2].value; // 기본재고상차 BOX
		arrData[5] 	= document.frm.base_stk_ea[tableLen-2].value; // 기본재고상차 EA
		arrData[6] 	= document.frm.brand_no[tableLen-2].value; // 전표번호
	}
	else {
...
function setLastRow() {
	...
	var tableLen = left_tbody.rows.length;
	if( document.frm.routeClass[tableLen-2] ) {
		document.frm.trans_date[tableLen-2].value 	= arrData[0];  // 수송일자
		document.frm.item_id[tableLen-2].value 		= arrData[1];  // 제품코드
		document.frm.item_name[tableLen-2].value 	= arrData[2];  // 제품명
		document.frm.base_stk_plt[tableLen-2].value = arrData[3];  // 기본재고상차 PLT
		document.frm.base_stk_box[tableLen-2].value = arrData[4];  // 기본재고상차 BOX
		document.frm.base_stk_ea[tableLen-2].value	= arrData[5];  // 기본재고상차 EA
		document.frm.brand_no[tableLen-2].value		= arrData[6]; // 전표번호
		
		divTransDate[tableLen-2].innerHTML 	= arrData[0]; // 수송일자
		divItemId[tableLen-2].innerHTML 	= arrData[1]; // 제품코드
		divItemName[tableLen-2].innerHTML 	= "&nbsp;" + arrData[2]; // 제품명
		divBasePlt[tableLen-2].innerHTML 	= fixedPoint(arrData[3], 2) + "&nbsp;";  // 기본재고상차 PLT
		divBaseBox[tableLen-2].innerHTML 	= arrData[4] + "&nbsp;";  				 // 기본재고상차 BOX
		divBaseEa[tableLen-2].innerHTML 	= arrData[5] + "&nbsp;";  				 // 기본재고상차 EA
		divBrandNo[tableLen-2].innerHTML 	= arrData[6] + "&nbsp;";  				 // 전표번호
...
//	}
	...
	var in_paramKey = "version!%!trans_date!%!src_loc!%!tgt_loc";
	var in_paramCode = version+"!%!"+trans_date+"!%!"+src_loc+"!%!"+tgt_loc;
...
function CalculTotal(lastIdx){
	...
	for(Idx ; Idx<lastIdx ; Idx++) {
		...
		if(document.frm.base_stk_plt[Idx]){
			base_stk_plt 		= Number(document.frm.base_stk_plt[Idx].value);
			base_stk_box 		= Number(document.frm.base_stk_box[Idx].value);
			base_stk_ea 		= Number(document.frm.base_stk_ea[Idx].value);
		}else{
			base_stk_plt 		= Number(document.frm.base_stk_plt.value);
			base_stk_box 		= Number(document.frm.base_stk_box.value);
			base_stk_ea 		= Number(document.frm.base_stk_ea.value);
		}
		total_plt 	= total_plt 	+ base_stk_plt;
		total_box 	= total_box 	+ base_stk_box;
		total_ea 	= total_ea 		+ base_stk_ea;
	
	}
	divTotal_plt.innerHTML 		= numberFormat(total_plt)+"&nbsp;"+"&nbsp;";
	divTotal_box.innerHTML 		= numberFormat(total_box)+"&nbsp;"+"&nbsp;";
	divTotal_ea.innerHTML 		= numberFormat(total_ea)+"&nbsp;"+"&nbsp;";

}function changeValue(obj){		var tgt_loc = obj.value;	var plan_type = document.frm.plan_type.options;				if(tgt_loc == "8913"|| tgt_loc == "8938") {	commonUtil.getSelQeury( "tgt_loc", tgt_loc, "rp_01015_trans_class_list_ps",{	callback:function(result){						//옵션 지우기 현재 select option 갯수만큼			for(var i = plan_type.length-1 ; i >=1 ; i--){				   		plan_type.options[i] =null;		  	}						//옵션 채우기 result 갯수만큼만			for(var i=0; i<result.length ; i++) {	 			   		plan_type.options[i] = new Option(result[i][1],result[i][0]);	  		}											}			});	}else{					commonUtil.getSelQeury( "tgt_loc", tgt_loc, "rp_01015_trans_class_list",{	callback:function(result){						//옵션 지우기 현재 select option 갯수만큼			for(var i = plan_type.length-1 ; i >=1 ; i--){				   		plan_type.options[i] =null;		  	}						//옵션 채우기 result 갯수만큼만			for(var i=0; i<result.length ; i++) {	 			   		plan_type.options[i] = new Option(result[i][1],result[i][0]);	  		}											}			});					}	}
