 ...
 * @author iCOMPIA CORP.                                                                                                     
 */                                                                                                                          
public class ip_05040_WMS_CloseInfo_Scm_list extends HttpServlet {                                                             
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {               
		...
		try {                                                                                                                
			...
			if (mode.equals("save")) //                                                                                    
				gdRes = doSave(gdReq);			
			if (mode.equals("Synchronize")) //                                                                                    
				gdRes = doSynchronize(gdReq);	
	...
	public GridData doQuery(GridData gdReq) throws Exception {                                                               
		...
		try {                                                                                                                
			...
			String insel_close_gubn  	= gdReq.getParam("insel_close_gubn");    
			//String insel_close_gubn_scm = gdReq.getParam("insel_close_gubn_scm");
			String in_sel_gubn 			= gdReq.getParam("in_sel_gubn"); 
			String insel_dc 			= gdReq.getParam("insel_dc");                                                            
			String item_id 				= gdReq.getParam("item_id"); 
			String user_id	  			= gdReq.getParam("user_id");			
		                                                                                                                 
			String paramKey   = "start_date!%!end_date!%!insel_close_gubn!%!in_sel_gubn!%!insel_dc!%!item_id!%!user_id";                                                                      
			String paramCode  = start_date+"!%!"+end_date+"!%!"+insel_close_gubn+"!%!"+in_sel_gubn+"!%!"+insel_dc+"!%!"+item_id+"!%!"+user_id;                                                                   
		
		                                                                 
			
			String query_id   = "ip_05040_WMS_CloseInfo_Scm_list";                                                             
		...
		try {
			...
			for (int i = 0; i < rowCount; i++) {
				...
				if( flag){
					...
					//---------------------------------------------------------------------------------------------	----------------------
					sql +="	SELECT 		    '" +		start_date												+ "' 	AS CNFM_DATE, 		\n";
	...
	public GridData doSynchronize(GridData gdReq) throws Exception{
		
		System.out.println("doSynchronize() start!!!");
		conn = databaseUtility.getConnection("t3sinc"); // DB Connection
		stmt = conn.createStatement(); // statement 객체 생성
		...
		try {
		    ...
		    //---------------------SP_SET_ROH_PR_IF() 실행-------------------------//
            System.out.println("call SP_IP_CR_WMS_DEFAULT_REASON() 실행!!!");
            String sql = "call SP_IP_CR_WMS_DEFAULT_REASON  ('CRE_DATA' , TRUNC(SYSDATE), 1, 1, 'NA', SYSDATE)" ;
            boolean resultSp = stmt.execute(sql);
            System.out.println("call SP_IP_CR_WMS_DEFAULT_REASON() 실행 결과 : " + resultSp);
            //------------------------------전송 상태 체크----------------------------------//
			
			
			gdRes.addParam("mode", "Synchronize");
			gdRes.setMessage("성공적으로 작업하였습니다.");
			gdRes.setStatus("true");
        ...
        System.out.println("doSynchronize() end!!!");
