...
public class expenseFactoryResult_list01 extends HttpServlet {
	...
	public GridData doQuery(GridData gdReq) throws Exception {
		...
		try {
			...
			s_query+="\n    select nvl(qq.cat06,'') as cat06, nvl(qq.cat06_name,'') as cat06_name, nvl(qq.item_id,'') as item_id, nvl(qq.item_name,'') as item_name, nvl(qq.spec,'') as spec, 1 seq, '수량' gubun, ";
			s_query+="\n           nvl(to_char(max(case when qq.plant_id = '1110' then qq.qty_by_cost end), '999,999,999'),'0') as anyang,                                                                          ";
			s_query+="\n           nvl(to_char(max(case when qq.plant_id = '1120' then qq.qty_by_cost end), '999,999,999'),'0') as anseong,                                                                         ";
			s_query+="\n           nvl(to_char(max(case when qq.plant_id = '1130' then qq.qty_by_cost end), '999,999,999'),'0') as anseong_u,                                                                       ";
			s_query+="\n           nvl(to_char(max(case when qq.plant_id = '1140' then qq.qty_by_cost end), '999,999,999'),'0') as asan,                                                                            ";
			s_query+="\n           nvl(to_char(max(case when qq.plant_id = '1160' then qq.qty_by_cost end), '999,999,999'),'0') as gumi,                                                                            ";
			s_query+="\n           nvl(to_char(max(case when qq.plant_id = '1170' then qq.qty_by_cost end), '999,999,999'),'0') as busan,                                                                           ";
			s_query+="\n           nvl(to_char(max(case when qq.plant_id = '1180' then qq.qty_by_cost end), '999,999,999'),'0') as noksan,                                                                          ";
			s_query+="\n           nvl(to_char(sum(qq.qty_by_cost), '999,999,999'),'0') as total                                                                                                                    ";
			...
			s_query+="\n               and ls.version = '"+p_version+"' --and ls.version = '20090326.22.56'                                                                                                                                           ";
			s_query+="\n               and ls.cat06 like '%"+p_res_code+"%'                                                                                                                                           ";
			s_query+="\n           ) qq                                                                                                                                                                            ";
			s_query+="\n     group by qq.cat06, qq.cat06_name, qq.item_id, qq.item_name, qq.spec                                                                                                                   ";
			s_query+="\n     union                                                                                                                                                                                 ";
			s_query+="\n    /* 비용기반 2차 공장할당결과 공장별 수량, Portion 조회 */                                                                                                                              ";
			s_query+="\n    select nvl(qq.cat06,'') as cat06, nvl(qq.cat06_name,'') as cat06_name, nvl(qq.item_id,'') as item_id, nvl(qq.item_name,'') as item_name, qq.spec, 2 seq, 'Portion' gubun,              ";
			s_query+="\n           nvl(to_char(round(max(case when qq.plant_id = '1110' then qq.qty_by_cost end)/sum(qq.qty_by_cost), 2)*100, '999,999,999'),'0')||'%' as anyang,                                        ";
			...
			s_query+="\n           nvl(to_char(round(max(case when qq.plant_id = '1130' then qq.qty_by_cost end)/sum(qq.qty_by_cost), 2)*100, '999,999,999'),'0')||'%' as anseong_u,                                     ";
			s_query+="\n           nvl(to_char(round(max(case when qq.plant_id = '1140' then qq.qty_by_cost end)/sum(qq.qty_by_cost), 2)*100, '999,999,999'),'0')||'%' as asan,                                          ";
			s_query+="\n           nvl(to_char(round(max(case when qq.plant_id = '1160' then qq.qty_by_cost end)/sum(qq.qty_by_cost), 2)*100, '999,999,999'),'0')||'%' as gumi,                                          ";
			s_query+="\n           nvl(to_char(round(max(case when qq.plant_id = '1170' then qq.qty_by_cost end)/sum(qq.qty_by_cost), 2)*100, '999,999,999'),'0')||'%' as busan,                                         ";
			s_query+="\n           nvl(to_char(round(max(case when qq.plant_id = '1180' then qq.qty_by_cost end)/sum(qq.qty_by_cost), 2)*100, '999,999,999'),'0')||'%' as noksan,                                        ";
			s_query+="\n           nvl(to_char(round(sum(qq.qty_by_cost)/sum(qq.qty_by_cost), 2)*100, '999,999,999'),'0')||'%' as total                                                                                  ";
			s_query+="\n      from (/* 비율 vs. 비용 기반 공장할당결과 비교 */                                                                                                                                     ";
			s_query+="\n            select ls.version, ls.cat06, ls.cat06_name, ls.item_id, ls.item_name, ls.spec, ls.seq, ls.plant_id, ls.qty_by_cost                                                             ";
			s_query+="\n              from fa_rate_cost_qty_comp ls                                                                                                                                                ";
			s_query+="\n             where ls.plant_id is not null                                                                                                                                                 ";
			...
			for( int i = 0 ; rs.next() ; i++ ) {
                ...
                gdRes.getHeader("gubun"  ).addValue(rs.getString(7), "");
                gdRes.getHeader("anyang"  ).addValue(rs.getString(8), "");
                gdRes.getHeader("anseong"  ).addValue(rs.getString(9), "");
                gdRes.getHeader("anseong_u"  ).addValue(rs.getString(10), "");
                gdRes.getHeader("asan"  ).addValue(rs.getString(11), "");
                gdRes.getHeader("gumi"  ).addValue(rs.getString(12), "");
                gdRes.getHeader("busan"  ).addValue(rs.getString(13), "");
                gdRes.getHeader("noksan"  ).addValue(rs.getString(14), "");
                gdRes.getHeader("total"  ).addValue(rs.getString(15), "");
 			}
