...
public class sc_01010_capaAssignMaterialCompare_list02_1 extends HttpServlet {
	...
	public GridData doQuery(GridData gdReq) throws Exception {
		...
		try {
			...
			s_query += "\n    /* Capa 조건에 따른 할당물량 비교 : 공장별 집계 (설비유형, 제품, 공장별 화면용) */                                                ";
			s_query += "\n    select NVL(null,' ') AS cat06, NVL(null,' ') AS cat06_name, NVL(null,' ') AS item_id, NVL(null,' ') AS item_name, NVL(null,' ') AS spec, NVL(ls.plant_id,' ') AS PLANT_ID, NVL(ls.plant_name,' ') AS PLANT_NAME,                          ";
			s_query += "\n           ROUND(TO_NUMBER(NVL(nc.nc_qty,0)),0) AS NC_QTY, TO_NUMBER(NVL(null,0)) AS NC_EA, TO_NUMBER(NVL(nc.nc_rate,0)) AS NC_RATE, ROUND(TO_NUMBER(NVL(nc.nc_amount,0)),0) AS NC_AMOUNT,                                                                         ";
			s_query += "\n           ROUND(TO_NUMBER(NVL(ca.ca_qty,0)),0) AS CA_QTY, TO_NUMBER(NVL(null,0)) AS CA_EA, TO_NUMBER(NVL(ca.ca_rate,0)) AS CA_RATE, ROUND(TO_NUMBER(NVL(ca.ca_amount,0)),0) AS CA_AMOUNT                                                                          ";
			s_query += "\n      from (/* Capa 제약없음 */                                                                                                       ";
