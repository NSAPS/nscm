...
public class expenseFactoryResult_popup extends HttpServlet {
	...
	public GridData doQuery(GridData gdReq) throws Exception {
		...
		try {
			...
			//더블클릭시 경로별 비용 TRACE화면에 받은 품목(item_id)와 타겟(tgt_loc) 값.
			String p_st_item_id = gdReq.getParam("p_st_item_id");
			String p_st_tgt_loc = gdReq.getParam("p_st_tgt_loc");
			...
			System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
			System.out.println("!!!!!!!!!!!!!!!p_st_item_id : "+p_st_item_id+"!!!!!!!!!!!!!!!!!");
			System.out.println("!!!!!!!!!!!!!!!p_st_tgt_loc : "+p_st_tgt_loc+"!!!!!!!!!!!!!!!!!");
			System.out.println("!!!!!!!!!!!!!!!  yongsoo  !!!!!!!!!!!!!!!!!!!");
			...
			s_query+="\n    /* 제품별 가능 경로별 비용 기준정보 */                                                                                                                ";
			s_query+="\n    select nvl(item_id,' ') as item_id, nvl(item_name,' ') as item_name, nvl(spec,' ') as spec, nvl(src_loc,' ') as src_loc, nvl(tgt_loc,' ') as tgt_loc, ";
			s_query+="\n           nvl(src_loc_name,' ') as src_loc_name, nvl(tgt_loc_name,' ') as tgt_loc_name,                                                                  ";
			s_query+="\n           decode(cost,null,0,cost) as cost,                                                                                                              ";
			s_query+="\n           decode(trans_cost,null,0,trans_cost) as trans_cost,                                                                                            ";
			s_query+="\n           decode(real_cost,null,0,real_cost) as real_cost                                                                                                ";
			s_query+="\n      from v_fa_item_dcpath_cost                                                                                                                          ";
			s_query+="\n     where item_id = '"+p_st_item_id+"'                                                                                                                   ";
			s_query+="\n       and tgt_loc = '"+p_st_tgt_loc+"'                                                                                                                   ";
			s_query+="\n     order by cost                                                                                                                                        ";
			...
			for( int i = 0 ; rs.next() ; i++ ) {
                gdRes.getHeader("item_id"  ).addValue(rs.getString(1), ""); //rs.getString(),"이부분" 은 숨겨진 값을 설정 WiseGrid.GetCellHiddenValue('ITEM_ID',0) 로 가져올 수 있음.
                gdRes.getHeader("item_name"  ).addValue(rs.getString(2), "");
                gdRes.getHeader("spec"  ).addValue(rs.getString(3), "");
                gdRes.getHeader("src_loc"  ).addValue(rs.getString(4), "");
                gdRes.getHeader("tgt_loc"  ).addValue(rs.getString(5), "");
                gdRes.getHeader("src_loc_name"  ).addValue(rs.getString(6), "");
                gdRes.getHeader("tgt_loc_name"  ).addValue(rs.getString(7), "");
                gdRes.getHeader("cost"  ).addValue(rs.getString(8), "");
                gdRes.getHeader("trans_cost"  ).addValue(rs.getString(9), "");
                gdRes.getHeader("real_cost"  ).addValue(rs.getString(10), "");
 			}
