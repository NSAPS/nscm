...
public class sc_01010_capaAssignMaterialCompare_list02 extends HttpServlet {
	...
	public GridData doQuery(GridData gdReq) throws Exception {
		...
		try {
			...
			s_query += "\n    /* Capa 조건에 따른 할당물량 비교 : 설비유형, 제품, 공장별 */                                                                                                  ";
			s_query += "\n    select ls.cat06, ls.cat06_name, ls.item_id, ls.item_name, im.spec, ls.plant_id, ls.plant_name,                                                                 ";
			s_query += "\n           ROUND(TO_NUMBER(NVL(nc.nc_qty,0)),0) AS NC_QTY, TO_NUMBER(NVL(nc.nc_qty,0)) * TO_NUMBER(NVL(im.qty,0)) AS NC_EA, TO_NUMBER(NVL(nc.nc_rate,0)) AS NC_RATE, ROUND(TO_NUMBER(NVL(nc.nc_amount,0)),0) AS NC_AMOUNT,                                                                                        ";
			s_query += "\n           ROUND(TO_NUMBER(NVL(ca.ca_qty,0)),0) AS CA_QTY, TO_NUMBER(NVL(ca.ca_qty,0)) * TO_NUMBER(NVL(im.qty,0)) AS CA_EA, TO_NUMBER(NVL(ca.ca_rate,0)) AS CA_RATE, ROUND(TO_NUMBER(NVL(ca.ca_amount,0)),0) AS CA_AMOUNT                                                                                         ";
			s_query += "\n      from (/* Capa 제약없음 */                                                                                                                                    ";
			s_query += "\n            select im.cat06, cd.cat06_name, ct.item_id, ct.item_name, ct.dcpath2_cd plant_id, ct.dcpath2 plant_name                                                ";
			...
			s_query += "\n               and ct.dcpath2_cd   like '%"+p_factory+"%'                                                                                                                 ";			
			...
			s_query += "\n           ) ls,                                                                                                                                                   ";
			s_query += "\n           (/* Capa 제약없음 */                                                                                                                                    ";
			s_query += "\n            select distinct im.cat06, cd.cat06_name, ct.item_id, ct.item_name, ct.dcpath2_cd plant_id, ct.dcpath2 plant_name,                                      ";
			s_query += "\n                   sum(ct.qty) over(partition by im.cat06, ct.item_id, ct.dcpath2_cd) nc_qty,                                                                      ";
			s_query += "\n                   round(sum(ct.qty) over(partition by im.cat06, ct.item_id, ct.dcpath2_cd)/sum(ct.qty) over (partition by im.cat06, ct.item_id), 2) nc_rate,      ";
			s_query += "\n                   sum(ct.total_amt) over(partition by im.cat06, ct.item_id, ct.dcpath2_cd) nc_amount                                                              ";
			s_query += "\n              from fa_cost_trace ct,                                                                                                                               ";
			s_query += "\n                   item_mst im,                                                                                                                                    ";
			s_query += "\n                   (select cd cat06, cd_name cat06_name                                                                                                            ";
			s_query += "\n                      from code_mst                                                                                                                                ";
			...
			s_query += "\n           ) nc,                                                                                                                                                   ";
			s_query += "\n           (/* Capa 제약 */                                                                                                                                        ";
			s_query += "\n            select distinct im.cat06, cd.cat06_name, ct.item_id, ct.dcpath2_cd plant_id, ct.dcpath2 plant_name,                                                    ";
			s_query += "\n                   sum(ct.qty) over(partition by im.cat06, ct.item_id, ct.dcpath2_cd) ca_qty,                                                                      ";
			s_query += "\n                   round(sum(ct.qty) over(partition by im.cat06, ct.item_id, ct.dcpath2_cd)/sum(ct.qty) over (partition by im.cat06, ct.item_id), 2) ca_rate,      ";
			s_query += "\n                   sum(ct.total_amt) over(partition by im.cat06, ct.item_id, ct.dcpath2_cd) ca_amount                                                              ";
			s_query += "\n              from fa_cost_trace ct,                                                                                                                               ";
			s_query += "\n                   item_mst im,                                                                                                                                    ";
			s_query += "\n                   (select cd cat06, cd_name cat06_name                                                                                                            ";
			s_query += "\n                      from code_mst                                                                                                                                ";
			...
			s_query += "\n           ) ca,                                                                                                                                                   ";
			s_query += "\n           item_mst im                                                                                                                                             ";
			s_query += "\n     where ls.cat06    = nc.cat06(+)                                                                                                                               ";
			s_query += "\n       and ls.item_id  = nc.item_id(+)                                                                                                                             ";
			s_query += "\n       and ls.plant_id = nc.plant_id(+)                                                                                                                            ";
			s_query += "\n       and ls.cat06    = ca.cat06(+)                                                                                                                               ";
			s_query += "\n       and ls.item_id  = ca.item_id(+)                                                                                                                             ";
			s_query += "\n       and ls.plant_id = ca.plant_id(+)                                                                                                                            ";
			s_query += "\n       and ls.item_id  = im.item_id(+)                                                                                                                             ";
			s_query += "\n     order by ls.cat06, ls.cat06_name, ls.item_id, ls.item_name, ls.plant_id, ls.plant_name                                                                        ";
			...
			for( int i = 0 ; rs.next() ; i++ ) {
                ...
                gdRes.getHeader("NC_QTY"  ).addValue(rs.getString(8), "");
                gdRes.getHeader("NC_EA"  ).addValue(rs.getString(9), "");
                gdRes.getHeader("NC_RATE"  ).addValue(rs.getString(10), "");
                gdRes.getHeader("NC_AMOUNT"  ).addValue(rs.getString(11), "");
                gdRes.getHeader("CA_QTY"  ).addValue(rs.getString(12), "");
                gdRes.getHeader("CA_EA"  ).addValue(rs.getString(13), "");
                gdRes.getHeader("CA_RATE"  ).addValue(rs.getString(14), "");
                gdRes.getHeader("CA_AMOUNT"  ).addValue(rs.getString(15), "");
 			}
